...
class Program {
  public int[] dijkstrasAlgorithm(int start, int[][][] edges) {
    // Write your code here.
		...
		for (int i = 0; i < len; i++)
			minDistancesPairs.add(new Item(i, Integer.MAX_VALUE));
	...
	private void replaceInfinities(int[] distances) {
		for (int idx = 0; idx < distances.length; idx++) {
			if (distances[idx] == Integer.MAX_VALUE)
				distances[idx] = -1;
		}
	}
    ...
    private void siftDown(int currentIdx, int endIdx, List<Item> heap) {
      ...
      if (
				leftIdx <= endIdx &&
				heap.get(leftIdx).distance < heap.get(currentIdx).distance
			)
				smallestIdx = leftIdx;

      if (
				rightIdx <= endIdx &&
				heap.get(rightIdx).distance < heap.get(smallestIdx).distance
			)
    ...
    private void siftUp(int currentIdx, List<Item> heap) {
      ...
      while (
				currentIdx > 0 &&
				heap.get(parentIdx).distance > heap.get(currentIdx).distance
			) {
