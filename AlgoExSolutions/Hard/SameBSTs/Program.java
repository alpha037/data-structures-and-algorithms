...
class Program {
   ...
   */
	private static boolean sameBsts(
		List<Integer> arrayOne, List<Integer> arrayTwo, int rootIdxOne,
		int rootIdxTwo, int minValue, int maxValue
	) {
	...
	private static int getFirstSmallerIdx(
		List<Integer> array, int startIdx, int minValue
	) {
	...
	private static int getFirstBiggerOrEqualIdx(
		List<Integer> array, int startIdx, int maxValue
	) {
		for (int i = startIdx + 1; i < array.size(); i++)
