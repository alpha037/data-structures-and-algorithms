    ...
    // Write your code here.
		...
		for (int i = 2; i < len; i++)
			maxAMinusBPlusC[i] = Math.max(
				maxAMinusBPlusC[i - 1], maxAMinusB[i - 1] + array[i]
			);
		
		int[] maxAMinusBPlusCMinusD = new int[len];
		maxAMinusBPlusCMinusD[0] = maxAMinusBPlusCMinusD[1] = maxAMinusBPlusCMinusD[2]
			= Integer.MIN_VALUE;
		for (int i = 3; i < len; i++)
			maxAMinusBPlusCMinusD[i] = Math.max(
				maxAMinusBPlusCMinusD[i - 1], maxAMinusBPlusC[i - 1] - array[i]
			);
