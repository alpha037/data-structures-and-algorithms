...
class Program {
  ...
  public int nonAttackingQueens(int n) {
    ...
    return getNonAttackingQueensPlacements(
			0, new HashMap<>(), new HashMap<>(), new HashMap<>(), n
		);
  }
	
	private int getNonAttackingQueensPlacements(
		int row, Map<Integer, Boolean> visitedColumns, 
		Map<Integer, Boolean> visitedUpDiagonals,
		Map<Integer, Boolean> visitedDownDiagonals, int n
	) {
		if (row == n) return 1;
		
		int totalPlacements = 0;
		for (int col = 0; col < n; col++) {
			if (isNotSafeToPlaceQueen(
				row, col, visitedColumns, visitedUpDiagonals, visitedDownDiagonals
			)) continue;
			placeQueen(
				row, col, visitedColumns, visitedUpDiagonals, visitedDownDiagonals
			);
			totalPlacements += getNonAttackingQueensPlacements(
	...
	private boolean isNotSafeToPlaceQueen(
		int row, int col, Map<Integer, Boolean> visitedColumns,
		Map<Integer, Boolean> visitedUpDiagonals,
		Map<Integer, Boolean> visitedDownDiagonals
	) {
		return
			visitedColumns.containsKey(col) ||
