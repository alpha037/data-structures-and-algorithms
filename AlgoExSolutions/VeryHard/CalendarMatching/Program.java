  ...
  public static List<StringMeeting> calendarMatching(
	...
	private static List<Integer[]> updateCalendar(
		...
		for (StringMeeting meeting : updatedCalendar)
			calendarInMins.add(new Integer[] {
	...
	private static List<Integer[]> mergeOverlappingIntervals(
		...
		for (int i = 1; i < calendar.size(); i++) {
			...
			if (previous[1] >= current[0]) {
				mergedIntervals.set(
					...
					new Integer[] {
						previous[0], Math.max(previous[1], current[1])
					}
				);
			}
	...
	private static List<StringMeeting> getMatchingAvailabilities(
		...
		for (int i = 1; i < calendar.size(); i++) {
			...
			if (end - start >= meetingDuration)
				matchingAvailabilities.add(new StringMeeting(
					minutesToTime(start),
					minutesToTime(end)
				));
		}
	...
	private static String minutesToTime(int minutes) {
		...
		String minutesString = mins < 10 ?
			"0" + String.valueOf(mins) : String.valueOf(mins);
