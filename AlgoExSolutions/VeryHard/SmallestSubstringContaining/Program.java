  ...
  public static String smallestSubstringContaining(
    ...
    // Write your code here.
		Map<Character, Integer>
			smallStringCount = getSmallStringCharCount(smallString);
		...
		while (rightIdx < len) {
			...
			if (smallStringCount.get(rightChar) == bigStringCount.get(rightChar))
				++charsFound;
			...
			while (totalChars == charsFound && leftIdx <= rightIdx) {
				...
				if (smallStringCount.get(leftChar) == bigStringCount.get(leftChar))
					--charsFound;
	...
	private static int[] getCloserBounds(int idx1, int idx2, int idx3, int idx4) {
		return idx2 - idx1 > idx4 - idx3 ?
			new int[] {idx3, idx4} : new int[] {idx1, idx2};
	}
}
