...
class Program {
  ...
  public boolean twoEdgeConnectedGraph(int[][] edges) {
    // Write your code here.
		...
		if (
			getMinimumArrivalTimeOfAncestors(
				sourceVertex, -1, 0, minimumArrivalTimes, edges
			) == -1
		) return false;
	...
	private int getMinimumArrivalTimeOfAncestors(
		int currentVertex, int parent, int currentTime,
		int[] minimumArrivalTimes, int[][] edges
	) {
		...
		for (int destination : edges[currentVertex]) {
			if (minimumArrivalTimes[destination] == -1)
				minimumArrivalTime = Math.min(
					minimumArrivalTime, getMinimumArrivalTimeOfAncestors(
						destination, currentVertex, currentTime + 1, minimumArrivalTimes,
						edges
					)
				);
			else if (destination != parent)
				minimumArrivalTime = Math.min(
					minimumArrivalTime, minimumArrivalTimes[destination]
				);
		}
		
		return
			currentTime == minimumArrivalTime &&
			parent != -1 ? -1 : minimumArrivalTime;
	}
