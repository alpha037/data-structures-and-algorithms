  ...
  public static int airportConnections(
    ...
    // Write your code here.
		...
		List<AirportNode> unreachableAirportNodes = getUnreachableAirportNodes(
			airports, airportGraph, startingAirport
		);
	...
	private static List<AirportNode> getUnreachableAirportNodes(
		List<String> airports, Map<String, AirportNode> airportGraph,
	...
	private static void dfsAirports(
		String airport, Map<String, AirportNode> airportGraph,
	...
	private static void markUnreachableConnections(
		...
		for (AirportNode airportNode : unreachableAirportNodes) {
			...
			dfsUnreachableConnections(
				airportNode.airport, airportGraph, unreachableConnections,
				new HashMap<>()
			);
	...
	private static void dfsUnreachableConnections(
		...
		if (
			visitedAirports.containsKey(airport) ||
			airportGraph.get(airport).isReachable
		) return;
		visitedAirports.put(airport, true);
		...
		for (String connection : airportGraph.get(airport).connections)
			dfsUnreachableConnections(
				connection, airportGraph, unreachableConnections, visitedAirports
			);
	}
