...
class Program {
  ...
  public int[][] aStarAlgorithm(
    ...
    // Write your code here.
		...
		startNode.estimatedDistanceToNode = calculateManhattanDistance(
			startNode, endNode
		);
		...
		while (!minHeap.isEmpty()) {
			...
			for (Node neighbour : getNeighbours(currentNode, nodes)) {
				...
				neighbour.estimatedDistanceToNode =
					tentativeDistanceToNeighbour + calculateManhattanDistance(
					neighbour, endNode
				);
	...
	private List<Node> getNeighbours(Node node, List<List<Node>> nodes) {
		...
		if (node.row < nodes.size() - 1)
			neighbours.add(nodes.get(node.row + 1).get(node.col));
		
		if (node.row > 0)
			neighbours.add(nodes.get(node.row - 1).get(node.col));
		
		if (node.col < nodes.get(0).size() - 1)
			neighbours.add(nodes.get(node.row).get(node.col + 1));
    ...
    public MinHeap(List<Node> array) {
			for (int i = 0; i < array.size(); i++)
				nodePositions.put(array.get(i).id, i);
    ...
    public void siftDown(int currentIdx, int endIdx, List<Node> heap) {
      // Write your code here.
			...
			if (
				leftIdx <= endIdx && heap.get(leftIdx).estimatedDistanceToNode <
				heap.get(currentIdx).estimatedDistanceToNode
			)
				smallestIdx = leftIdx;
			
			if (
				rightIdx <= endIdx && heap.get(rightIdx).estimatedDistanceToNode <
				heap.get(smallestIdx).estimatedDistanceToNode
			)
    ...
    public void siftUp(int currentIdx, List<Node> heap) {
      // Write your code here.
			...
			while (
				currentIdx > 0 && heap.get(parentIdx).estimatedDistanceToNode >
				heap.get(currentIdx).estimatedDistanceToNode
			) {
